...
<div class="max-w-2xl mx-auto bg-gradient-to-br from-blue-950 via-slate-900 to-blue-900 text-white rounded-3xl p-6 lg:p-8 shadow-2xl border border-blue-900 relative overflow-hidden">
    ...
    <header class="mb-6 relative z-10 border-b border-blue-800 pb-4 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-wide flex items-center">
                <i class="bi bi-pencil-square text-blue-400 mr-2"></i>Edit Akun Pengguna
            </h1>
            <p class="text-gray-300 text-xs mt-1">Ubah informasi akun dan hak akses untuk {{ $user->name }}.</p>
        </div>
    ...
    <form method="POST" action="{{ route('users.update', $user->id) }}" class="space-y-5 relative z-10">
        @csrf
        @method('PUT')
        ...
        <div class="border-t border-blue-900/80 pt-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 bg-slate-950/40 p-4 rounded-2xl border border-blue-900/60">
                @php
                    $currentMenus = is_null($user->allowed_menus)
                        ? ($user->isSuperAdmin() ? ['dashboard', 'stu_unit', 'stok_unit', 'digital_marketing', 'cabang', 'users'] : ['dashboard', 'stu_unit', 'stok_unit', 'digital_marketing'])
                        : ($user->allowed_menus ?? []);
                    $oldMenus = old('allowed_menus', $currentMenus);
                @endphp
        ...
        <div class="border-t border-blue-900/60 pt-4">
            <p class="text-xs font-semibold text-blue-300 mb-2">Ganti Password (Opsional)</p>
            <p class="text-[11px] text-gray-400 mb-3">Kosongkan kolom password jika Anda tidak ingin mengubah password user ini.</p>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label for="password" class="block text-xs font-bold uppercase tracking-wider text-gray-300 mb-1.5">Password Baru</label>
                    <input type="password" name="password" id="password" placeholder="Kosongkan jika tidak diganti"
                           class="w-full bg-slate-950/60 border border-blue-800 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                </div>

                <div>
                    <label for="password_confirmation" class="block text-xs font-bold uppercase tracking-wider text-gray-300 mb-1.5">Konfirmasi Password Baru</label>
        ...
        <div class="pt-4 flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="px-5 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white shadow-lg hover:shadow-blue-500/25 transition">
                <i class="bi bi-save mr-1"></i> Perbarui Akun
            </button>
        </div>
    </form>
</div>
